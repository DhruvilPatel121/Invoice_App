...
API ID: api-n9QVxo8blgrL Endpoint: GET https://app-cwzg4szdxkox-api-n9QVxo8blgrL.gateway.appmedo.com/v1/images/omni-image/{task_id}
...
Call submitOmniImageTask() to submit the task and obtain a task_id
Repeatedly call queryOmniImageTask(taskId) at 7-second intervals until task_status is succeed or failed
Retrieve the URL list from task_result.images (single) or task_result.series_images (series)
For each image, use the Bash tool to run curl -L -o <local-path> "<url>" to download it locally
