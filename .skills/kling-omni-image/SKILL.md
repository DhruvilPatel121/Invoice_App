...
Call the submit endpoint (POST) to create a generation task and obtain a task_id
Poll the query endpoint (GET) until task_status becomes succeed or failed
Retrieve image URLs from task_result.images (single mode) or task_result.series_images (series mode)
Transfer image URLs to Supabase Storage (CDN links expire after 30 days)
Read references/submit-api.md for the full specification and code for the submit endpoint. Read references/query-api.md for the full specification and code for the query endpoint.
...
// Step 1: Submit task
const submitResp = await fetch("https://app-cwzg4szdxkox-api-2Y00Vzbe0MBY.gateway.appmedo.com/v1/images/omni-image", {
method: "POST",
headers: {
...
await new Promise(r => setTimeout(r, POLL_INTERVAL_MS));
    ...
    const queryResp = await fetch(
      `https://app-cwzg4szdxkox-api-n9QVxo8blgrL.gateway.appmedo.com/v1/images/omni-image/${taskId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
Call generateOmniImage() to execute the submit → poll loop and obtain the list of image URLs
For each image, use the Bash tool to run curl -L -o <local-path> "<url>" to download the file locally
Notify the user of the file paths where images have been saved
