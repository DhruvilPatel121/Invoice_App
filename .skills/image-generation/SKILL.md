...
Read references/submit-api.md for the full submit endpoint spec. Read references/query-api.md for the full query/poll endpoint spec.
...
const POLL*INTERVAL_MS = 7000; // 5–10 s recommended
const TIMEOUT_MS = 10 * 60 \_ 1000; // 10 minutes
...
Call submitTask(contents) to submit the generation job → receive taskId
Poll queryTask(taskId) every 7 seconds until status === "SUCCESS", "FAILED", or "TIMEOUT" (timeout 10 min)
Extract the Base64 string from candidates[0].content.parts[0].text (strip the ![image](data:image/jpeg;base64, prefix and the closing ))
Use Bash tool: echo "<base64>" | base64 -d > <local-path>.jpg
...
Frontend calls the image-generation-submit Edge Function → receives taskId
Frontend polls the image-generation-query Edge Function every 7 seconds
On SUCCESS, the Edge Function decodes the Base64 and uploads to Supabase Storage, returning a persistent publicUrl
Frontend displays the image from publicUrl
Billing
Item Price
