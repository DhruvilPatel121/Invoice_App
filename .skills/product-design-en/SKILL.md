...
Confirm product requirements, target users, and core functionality
Generate 3-5 visual directions from different angles
Each direction includes: design concept, layout approach, and use case 2. Prototyping
...
Confirm scope (page/flow/component)
Determine fidelity level (low/mid/high)
Build interactive prototype with interaction logic and state changes documented 3. URL to Code
...
Generate editable local code (HTML/CSS/components)
Mark areas for modification 4. Image to Code
...
Confirm design screenshot or mockup
Analyze layout structure, component hierarchy, and styles
Generate responsive code matching the design as closely as possible 5. UX Audit
...
Confirm review scope (page/flow/feature)
Analyze by dimension: information architecture, navigation, interaction feedback, content clarity, consistency
Output prioritized issue list with improvement suggestions 6. Accessibility Review
...
Check key items: color contrast, keyboard navigation, screen reader support, ARIA labels, focus management
Output accessibility issue list with remediation suggestions 7. Design QA
...
Check: pixel-perfect accuracy, spacing/typography consistency, interaction state completeness, responsive adaptation
Output QA report: passed items, items to fix, optimization suggestions 8. User Research
...
Confirm research goals and scope
Analyze provided materials (user feedback, data, competitive research)
Output research findings, user insights, and design recommendations 9. Context Gathering
...
Clarify product positioning, target users, and core scenarios
Collect existing design assets, brand guidelines, and constraints
Output unified design context document for subsequent use 10. Sharing
...
Confirm sharing goals and audience
Prepare export format or share link
Add review notes and key feedback questions 11. User Context Management
...
Confirm user preferences or context to save
Record key information (brand assets, design tokens, color schemes)
Auto-apply saved preferences in subsequent design work
...
User Input First: All design work must be based on user-provided product and design context. The Agent must not fabricate design decisions.
Source Attribution: When citing design materials, note the source. Conclusions without sources must be labeled as speculative.
Deliverable Output: Final output must be a ready-to-use artifact (prototype code, review report, design proposal), not a process explanation.
Call Invisibility Constraint: Workflow execution is internal only. Output only the final design artifact.
Reference Templates
...
Common Pitfalls
This Skill does not connect to Figma or any design tool API. All design materials depend on user-provided context.
Design code examples (HTML/CSS) should adapt to the frontend stack available in the current session.
Screenshot-to-code accuracy is affected by screenshot quality. Inform the user about potential deviations with low-resolution images.
Accessibility reviews are based on WCAG 2.1 AA standards.
When ideating, always present multiple directions for user selection. Never propose only one option.
