name:security-and-permissions-review
description:检查应用的权限、输入校验、敏感信息、认证和常见安全风险。借鉴 Addy 的 security-and-hardening，只输出防御性检查清单和修复建议，适合应用上线前的安全自查。当用户需要检查应用安全性、审查权限模型、验证输入校验、排查敏感信息泄露或进行安全自查时触发。
license:MIT
...
const MAX_SIZE = 5 _ 1024 _ 1024; // 5MB
