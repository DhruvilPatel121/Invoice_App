...
Capability Overview
Item Value
Endpoint POST https://app-cwzg4szdxkox-api-rLobRWZ712b9.gateway.appmedo.com/v1beta/models/gemini-3.1-flash-image-preview:generateContent
...
Obtain the source image as a Base64 string (read file → encode, or accept from user).
Call the generation endpoint with text instruction + inlineData (mimeType + base64 data).
Extract candidates[0].content.parts — find the entry where inlineData exists (skip thought: true entries).
Decode and save the Base64 PNG to disk immediately (data exists only in this response).
Text-to-image (no source image)
Call the generation endpoint with only a text part — omit inlineData.
Same extraction and save steps as above.
...
// POST to: https://app-cwzg4szdxkox-api-rLobRWZ712b9.gateway.appmedo.com/v1beta/models/gemini-3.1-flash-image-preview:generateContent
...
Call the generation function and extract inlineData.data from candidates[0].content.parts
Use the Bash tool to decode and save the Base64 data: echo "<base64>" | base64 -d > <local_path>.png
Notify the user that the file has been saved to the corresponding path
