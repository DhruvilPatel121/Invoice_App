...
Confirm account name and meeting time
Organize brief based on user-provided context
Output structure: Account background → Relationship history → Key contacts → Meeting objectives → Recommended approach 2. Post-Call Follow-Up
...
Confirm key conclusions and action items
Generate internal follow-up summary (key points, decisions, action items)
Draft customer follow-up email
Generate CRM note suggestions 3. Account Prioritization
...
Collect account list and key information
Rank by multiple dimensions: stage, deal size, urgency
Output prioritized list with recommended actions 4. Forecast Review
...
Confirm review scope and period
Analyze each deal's risk level
Output recommendations: deals to watch, deals at risk of slipping, deals needing additional resources 5. Deal Strategy Planning
...
Confirm account and deal stage
Map buying committee (decision-makers, influencers, end users)
Identify procurement risks (budget, timeline, competition)
Output path forward and prioritized next actions 6. Competitive Intelligence
...
Confirm target competitor(s)
Multi-dimensional comparison: product, pricing, market share, strengths/weaknesses
Output competitive battlecard with common objection responses 7. Account Enrichment
...
Organize basic company info from provided materials
Supplement contact details, tech stack, recent developments 8. Internal Source Discovery
...
Clarify the topic/need
Recommend relevant internal resources and documents based on user-provided context 9. Account Signal Monitoring
...
Confirm target account list
Analyze recent developments (personnel changes, funding, new products, industry news)
Output updated account intelligence with recommended actions 10. Call Coaching
...
Confirm call recording or key points
Analyze key stages: opening, discovery, presentation, objection handling, closing
Output improvement suggestions with reference talk tracks 11. Call Trend Analysis
...
Confirm time range and call sample
Compare performance stage by stage across calls
Output trend analysis: improvements, regressions, stable patterns, coaching recommendations 12. Customer Quote Retrieval
...
Confirm theme/keywords
Search provided transcripts or notes for relevant customer quotes
Attribute sources and note confidence level 13. Company Deep Research
...
Multi-dimensional research: financials, product, technology, competitive landscape, customer reviews
Output research report with key findings recorded 14. Next-Step Suggestions
...
Review recent sales activities and progress
Identify pending items and follow-up opportunities
Recommend 1-3 prioritized next steps 15. User Preference Management
...
Confirm preferences or settings to save
Record key context (account groupings, communication style, preferred templates, etc.)
Auto-apply saved preferences in subsequent conversations
...
User Input First: All workflows must be based on user-provided account information and context. The Agent must not fabricate data.
Source Attribution: When citing user-provided materials, note the source. Conclusions without sources must be explicitly labeled as speculative.
Deliverable Output: Final output must be a ready-to-use sales artifact (brief, email, strategy doc, etc.), not a process explanation.
Call Invisibility Constraint: Workflow execution is internal only. Do not display step details or intermediate results; output only the final sales material.
Communication Rules
...
Common Pitfalls
This Skill does not connect to any CRM or external data sources. All account information depends on user-provided context.
If the user has not provided sufficient account background, ask clarifying questions before generating. Do not fill with generic templates.
Data and financial figures in sales materials must come from the user. The Agent must not fabricate amounts.
For pricing, discounts, and other sensitive information, use only the exact values provided by the user. Do not add unconfirmed quotes.
When competitive analysis involves competitor information, clearly label the information source (user-provided / public knowledge / inference).
