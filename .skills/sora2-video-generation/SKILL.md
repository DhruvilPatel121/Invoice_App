...
Query Status POST https://api-DYJwnoM46d6a@app-cwzg4szdxkox-api-DYJwnoM46d6a.gateway.appmedo.com
...
Read references/create-video-api.md for the full Create Video spec and code. Read references/query-video-api.md for the full Query Status spec and code. Read references/reference-video-api.md for the Reference Image Video spec and code. Read references/remix-video-api.md for the Remix Video spec and code.

End-to-End Workflow (Async Polling)
All video generation endpoints are asynchronous. The standard workflow is:

Submit — call the Create Video (or Reference/Remix) endpoint; receive { id, status: "queued" }.
Poll — call the Query endpoint every ~7 seconds with the returned id.
Result — when status === "completed", the response includes video_url. Download it immediately (CDN links are ephemeral).
...
const TIMEOUT_MS = 10 _ 60 _ 1000; // 10-minute hard timeout
...
Call the Create Video endpoint to get the id
Poll the Query endpoint until status === "completed" and extract video_url
Use the Bash tool to run curl -L -o <local_path> "<video_url>" to download the video
