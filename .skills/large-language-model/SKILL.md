...
Endpoint: POST https://app-cwzg4szdxkox-api-VaOwP8E7dJqa.gateway.appmedo.com/v1beta/models/gemini-2.5-flash:streamGenerateContent?alt=sse
...
End-to-End Workflow
The client sends the conversation history (contents array) to the Edge Function.
The Edge Function reads INTEGRATIONS_API_KEY from the Deno environment, attaches the X-Gateway-Authorization header, and calls the upstream endpoint via POST.
The upstream responds with an SSE stream; the Edge Function passes the stream through directly without buffering.
The frontend consumes the stream with ky + eventsource-parser and renders Markdown with streamdown.
