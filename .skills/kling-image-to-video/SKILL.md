...
Read references/create-task-api.md for the full submit-task specification and Edge Function code. Read references/query-task-api.md for the full query-task specification and Edge Function code.
...
const response = await fetch("https://app-cwzg4szdxkox-api-eLMlJj3KJD89.gateway.appmedo.com/v1/videos/image2video", {
method: "POST",
headers: {
...
await new Promise(r => setTimeout(r, POLL_INTERVAL_MS));
    ...
    const res = await fetch(
      `https://app-cwzg4szdxkox-api-rLobzpqX85m9.gateway.appmedo.com/v1/videos/image2video/${taskId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
Call submitImage2VideoTask to obtain the task_id
Call pollUntilDone to poll until status is succeed and obtain the video URL
Use the Bash tool to run curl -L -o <local-path>.mp4 "<url>" to download the video locally
Inform the user of the file path where the video has been saved
...
Read references/create-task-api.md for the complete submit Edge Function (edge-functions/kling-submit-image2video.ts). Read references/query-task-api.md for the complete query Edge Function (edge-functions/kling-query-image2video.ts) including Supabase Storage transfer.
...
const POLL_INTERVAL_MS = 7000;
const TIMEOUT_MS = 10 _ 60 _ 1000;
const deadline = Date.now() + TIMEOUT_MS;
