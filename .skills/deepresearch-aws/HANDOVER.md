...
https://app-cwzg4szdxkox-api-zYm4X4jwv0bL.gateway.appmedo.com/
3.2 monitor.py
...
评估输入 — 检查是否缺少研究目的/时间范围/输出格式，缺太多则追问
增强 Query — 改写为流畅的研究简报 + 加语言前缀（如 深度研究以下内容：）
启动 + 监控 — 后台启动 deepresearch.py，循环调用 monitor.py 检查进度
读取结果 — 完成后 cat 结果文件，原样输出给用户
四、已知问题与待处理项
...
递增但在 SKILL.md 中明确要求设置 Bash timeout 参数为 (sleep_seconds + 60) \* 1000 毫秒
把 sleep 起始值改小（如 30s 起步，每次 +5s），递增但永远不超过 110s
使用 run_in_background 模式执行 sleep + monitor，不受 120s 限制
...
七、调试建议
查看原始 API 返回：暂时保留 DEBUG_CHUNK 行，启动命令改为：
...
本地测试：需设置 INTEGRATIONS_API_KEY 环境变量，然后直接运行：
...
zip -r deepresearch-aws.zip deepresearch-aws/ -x "_.DS_Store" -x "_.pos"
部署到平台时上传该 zip 即可。
