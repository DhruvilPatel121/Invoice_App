...
Overview
Item Details
Create Task Endpoint POST https://app-cwzg4szdxkox-api-DY8MnRlwkXKa.gateway.appmedo.com/v1/images/generations
Query Task Endpoint GET https://app-cwzg4szdxkox-api-M9v0wzOkZXGY.gateway.appmedo.com/v1/images/generations/{task_id}
...
Call createImageTask() to submit the generation request and obtain a task_id
Poll queryImageTask(taskId) until the task completes
...
"https://app-cwzg4szdxkox-api-DY8MnRlwkXKa.gateway.appmedo.com/v1/images/generations",
{
method: "POST",
headers: {
...
const response = await fetch(
`https://app-cwzg4szdxkox-api-M9v0wzOkZXGY.gateway.appmedo.com/v1/images/generations/${taskId}`,
{
method: "GET",
headers: {
"Content-Type": "application/json",
...
Call createImageTask() to submit the task and obtain task_id
Call queryImageTask(taskId) every 7 seconds until task_status === "succeed" or "failed"
Extract image URLs from task_result.images[].url
Run curl -L -o <local-path>.jpg "<url>" via the Bash tool to download the image locally
