...
Capability Overview
Item Details
Create Task Endpoint POST https://app-cwzg4szdxkox-api-GYX1bbkRQj4a.gateway.appmedo.com/v1/images/editing/expand
Query Task (Single) Endpoint GET https://app-cwzg4szdxkox-api-AalZkkAG5w7L.gateway.appmedo.com/v1/images/editing/expand/{task_id}
Query Task (List) Endpoint GET https://app-cwzg4szdxkox-api-pLVzAAkGZwDL.gateway.appmedo.com/v1/images/editing/expand
...
const TIMEOUT_MS = 10 _ 60 _ 1000; // 10 minutes
...
Call createExpandTask() to submit the expansion request and obtain a task_id
Poll queryExpandTask(taskId) until the task completes
Download the image immediately using curl
...
"https://app-cwzg4szdxkox-api-GYX1bbkRQj4a.gateway.appmedo.com/v1/images/editing/expand",
{
method: "POST",
headers: {
...
const response = await fetch(
`https://app-cwzg4szdxkox-api-AalZkkAG5w7L.gateway.appmedo.com/v1/images/editing/expand/${taskId}`,
{
method: "GET",
headers: {
"Content-Type": "application/json",
...
Call createExpandTask() to submit the task and obtain task_id
Call queryExpandTask(taskId) every 7 seconds until task_status === "succeed" or "failed"
Extract the image URL from task_result.images[].url
Use the Bash tool to run curl -L -o <local-path>.jpg "<url>" to download the image locally
Inform the user that the file has been saved to the corresponding path
